...
test('prune keeps only the newest MAX_ITEMS', async () => {
  for (let i = 0; i < 30; i++) {
    await putItem({
      id: `it-${i}`,
      name: 'x',
  ...
  expect(all.length).toBe(24) // MAX_ITEMS
  expect(all.some((i) => i.id === 'it-29')).toBe(true) // newest kept
  expect(all.some((i) => i.id === 'it-0')).toBe(false) // oldest dropped
})
